...
void (*sort_func_ptr)(int*, int *, int, int, int, int);
...
void Init_PosMap (int posmap_size) {
    ...
    oram_controller.pos_map.entries = (PosMap_Entry*) calloc(sizeof(PosMap_Entry), posmap_size);
...
void Init_ORAM_Tree (int bucket_size, int oram_depth, int block_size) {
    ...
    oram_tree.oram  = (int*) calloc((sizeof(ORAM_Block) + block_size * sizeof(int)), bucket_size * (1 << oram_depth));
...
void Init_ORAM (int bucket_size, int num_real_blocks, int block_size, int C, enum Sort_Scheme sort_scheme) {
    ...
    if (sort_scheme == MERGE_SORT)
        sort_func_ptr = &MergeSort_TwoArray;
    else if (sort_scheme == BITONIC_SORT)
        sort_func_ptr = &BitonicSort_TwoArray;
    else
        assert(0);
    ...
    for (int i = 0; i < oram_tree.depth; i++) {
        for (int j = 0; j < oram_controller.stash.size; j++) {
            for (int k = 0; k < oram_controller.bucket_size; k++) {
                ORAM_Block* oram_block = &oram_tree.oram[(bucket_idx * oram_controller.bucket_size + k) * (sizeof(ORAM_Block) / sizeof(int) + oram_controller.block_size)];
                int*        oram_data  = &oram_tree.oram[(bucket_idx * oram_controller.bucket_size + k) * (sizeof(ORAM_Block) / sizeof(int) + oram_controller.block_size) + sizeof(ORAM_Block) / sizeof(int)];

                int if_move = oram_block->valid && !oram_controller.stash.info[j].valid;
                // dump matadata from ORAM_Tree to Stash
                cmov (if_move, &(oram_block->valid), &(oram_controller.stash.info[j].valid));
                cmov (if_move, &(oram_block->id),    &(oram_controller.stash.info[j].id));
                cmov (if_move, &(oram_block->leaf),  &(oram_controller.stash.info[j].leaf));
                cmov (if_move, &zero,                &(oram_block->valid));
                // dump data from ORAM_Tree to Stash
                cmovn(if_move, oram_data, &oram_controller.stash.data[j*oram_controller.block_size], oram_controller.block_size);
            }
        }
...
void Store_Blocks(int access_leaf){
    ...
    (*sort_func_ptr)((int*)oram_controller.stash.info, oram_controller.stash.data, oram_controller.stash.size, 1, sizeof(Stash_Info) / sizeof(int), oram_controller.block_size);
    ...
    for (int i = 0; i < oram_tree.depth; i++){
        for (int j = 0; j < oram_controller.bucket_size; j++){
            ...
            cmov (if_move, &oram_controller.stash.info[mov_stash_idx].valid, &oram_block->valid); // do not movve pointer
            cmov (if_move, &oram_controller.stash.info[mov_stash_idx].id,    &oram_block->id); // do not movve pointer
            cmov (if_move, &oram_controller.stash.info[mov_stash_idx].leaf,  &oram_block->leaf); // do not movve pointer
            cmov (if_move, &zero, &(oram_controller.stash.info[mov_stash_idx].valid));
            cmovn(if_move, &oram_controller.stash.data[mov_stash_idx * oram_controller.block_size], oram_data, oram_controller.block_size);
