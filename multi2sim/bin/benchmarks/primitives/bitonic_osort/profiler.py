...
BENCHMARK_DIR   = "/home/Jiyong_Yu/Research/OISA/primitives/bitonic_osort/"
BENCHMARK_NAME  = "osort"
BENCHMARK_PATH  = BENCHMARK_DIR + BENCHMARK_NAME + ".c"
EXECUTABLE_PATH  = BENCHMARK_DIR + BENCHMARK_NAME

TRACE_PACKAGE   = BENCHMARK_DIR + "trace.gz"
TRACE_FILE      = BENCHMARK_DIR + "trace"
DISASM_FILE     = BENCHMARK_DIR + "disasm"
EXE_INSTR_FILE  = BENCHMARK_DIR + "debug_isa.no_sim_flag"
...
def generateFile(param):
    ...
    ## compile C program
    os.system("gcc -m32 " + BENCHMARK_PATH + " -o " + EXECUTABLE_PATH + " " + CXX_FLAGS)

    # Generate disassembly
    with contextlib.suppress(FileNotFoundError):
        os.remove(DISASM_FILE)
    os.system(MULTI2SIM_PATH + " --x86-disasm " + EXECUTABLE_PATH + " > " + DISASM_FILE)

    ## Generate debug isa file
    with contextlib.suppress(FileNotFoundError):
        os.remove(EXE_INSTR_FILE)
    os.system(MULTI2SIM_PATH + " --x86-debug-isa " + EXE_INSTR_FILE + " " + EXECUTABLE_PATH + " " + param)

    ## Generate trace file
    with contextlib.suppress(FileNotFoundError):
        os.remove(TRACE_PACKAGE)
    os.system(MULTI2SIM_PATH + " --x86-sim detailed --trace " + TRACE_PACKAGE + " " + EXECUTABLE_PATH + " " + param)
...
def calcCommitTime():
    ...
    for instr_tup in committed_instrs:
        ...
        if addr in addr_map:
            ...
            if op == 'enter':
                ...
                # print ("push function", func, "at address ", addr)
                ...
                # for i in reversed(call_stack[-1]):
                    # print("pop function ", i, "at address ", addr)
    ...
    # print ("transit_Stack is: ", transit_stack)
...
for i in range(6):
    exp = i + 10;
    print ("N = ", 2 ** exp, " order = 1, seed = ", i)
    sys.stdout.flush()
    generateFile(str(exp) + " 1 " + str(i))
    calcCommitTime()

    print ("N = ", 2 ** exp, " order = 0, seed = ", i)
    sys.stdout.flush()
    generateFile(str(exp) + " 0 " + str(i))
    calcCommitTime()
